    ...
    return (
        <div className="relative" ref={bellRef}>
            <button
                ...
                className={`relative p-2 rounded-full transition-all duration-300 ${isOpen ? 'bg-foreground/10 text-foreground' : 'text-muted-foreground hover:text-foreground hover:bg-foreground/5'}`}
            >
            ...
            {isOpen && (
                ...
                >
                    <div className="p-4 border-b border-border bg-foreground/5 flex items-center justify-between">
                        ...
                        <span className="text-[10px] bg-foreground/5 text-muted-foreground px-2 py-0.5 rounded font-medium">Recent</span>
                    </div>
                    <div className="max-h-[400px] overflow-y-auto">
                        {notifications.length > 0 ? (
                            <div className="divide-y divide-border">
                                {notifications.map((notif: any, i) => (
                                    <div key={i} className="p-4 hover:bg-foreground/5 transition-colors flex gap-3">
                                        <div className={`mt-1 h-2 w-2 rounded-full shrink-0 ${notif.type === 'PAYMENT' ? 'bg-emerald-500' :
                                            notif.type === 'LIKE' ? 'bg-pink-500' :
                                                'bg-blue-500'
                                            }`} />
                                        <div>
                                            <p className="text-xs text-foreground/80 leading-relaxed mb-1">{notif.message}</p>

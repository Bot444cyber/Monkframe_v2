        ...
        >
            {loading ? (
                ...
                <div className="flex flex-col sm:flex-row w-full h-full">
                    <Link href={`/${(trendingItem.category || 'mockups').toLowerCase()}/${trendingItem.slug || trendingItem.id}`} onClick={onClose} className="sm:w-64 bg-blue-600 flex flex-col justify-between shrink-0 relative overflow-hidden group">
                        ...
                        {otherItems.length > 0 ? (
                            otherItems.map((item, ii) => (
                                <div key={ii} className="flex flex-col gap-1">
                                    <Link href={`/${(item.category || 'mockups').toLowerCase()}/${item.slug || item.id}`} onClick={onClose} className="group block">

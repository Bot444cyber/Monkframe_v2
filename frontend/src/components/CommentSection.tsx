...
const CommentSection: React.FC<CommentSectionProps> = ({
    ...
    const content = (
        <div className={`flex flex-col bg-card border-border transition-colors duration-500 ${variant === 'modal' ? 'w-full max-w-md h-full border-l shadow-2xl' : 'w-full rounded-3xl border bg-card/50'}`}>
            {/* Header */}
            <div className="p-6 border-b border-border flex items-center justify-between">
                <h3 className="text-xl font-bold text-foreground">Comments ({totalItems})</h3>
            ...
            <div className={`flex-1 overflow-y-auto p-6 space-y-6 ${variant === 'embedded' ? 'max-h-[600px] min-h-[300px]' : ''}`}>
                {loading ? (
                    <div className="text-center text-muted-foreground py-10">Loading comments...</div>
                ) : comments.length > 0 ? (
                    comments.map((comment) => (
                        <div key={comment.id} className="flex gap-3 group relative">
                            <div className="w-8 h-8 rounded-full bg-primary/20 flex items-center justify-center text-xs font-bold text-primary border border-primary/10 shrink-0">
                                {comment.user.full_name?.charAt(0) || '?'}
                            </div>
                            <div className="flex-1">
                                <div className="flex items-center justify-between mb-1">
                                    <span className="text-sm font-bold text-foreground">{comment.user.full_name || 'Unknown User'}</span>
            ...
            <div className="p-4 border-t border-border bg-secondary/5">
                {user ? (
                    <form onSubmit={handleSubmit}>
                        <div className="relative">
                            <input
                                ...
                                className="w-full bg-secondary/50 border border-border rounded-xl px-4 py-3 text-sm text-foreground focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/50 placeholder:text-muted-foreground/40 pr-12 transition-all"
                            />
                            <button
                                type="submit"
                                disabled={!newComment.trim()}
                                className="absolute right-2 top-1/2 -translate-y-1/2 p-1.5 bg-primary rounded-lg text-primary-foreground disabled:opacity-50 disabled:bg-secondary transition-all hover:bg-primary/90"
                            >
                    ...
                    <div className="text-center py-2">
                        <p className="text-sm text-muted-foreground mb-3">Please login to join the discussion</p>
                        <Link href="/login" className="inline-block px-6 py-2 rounded-lg bg-primary hover:bg-primary/90 text-primary-foreground text-sm font-bold transition-colors">
                            Login
                        </Link>
                    </div>
                )}
